...
class PlaySoundsViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        let saveSound:Float? = UserDefaults.standard.float(forKey: "saveSound")
        if saveSound != 0{
            self.sliderView.value = UserDefaults.standard.float(forKey: "saveSound")
        }
        else{
            print("no sound float ever been saved before")
        }
    }
    ...
    @IBAction func playAudio(_ sender: UIButton) {
        ...
        UserDefaults.standard.set(pitch, forKey: "saveSound")
        // Play the sound
